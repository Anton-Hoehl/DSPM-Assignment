...
The API key is store in a seperate textfile to protect it from unauthorized access. The file containing the key is also not submitted to github to protect it. 
...
To download all of the venue data on the ticketmaster page we need to iterate over all of the pages containing the information. The following for loop does exactly that and merges the results of each request. Checking on the website shows that we should have exactly 12238 entries as of the 11.02.2021. A conservative request rate with  4 second breaks between requests has been set.
...
The following plot shows the results for France.
